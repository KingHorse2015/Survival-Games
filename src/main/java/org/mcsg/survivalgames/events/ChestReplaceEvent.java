...
public class ChestReplaceEvent implements Listener {
	 ...
	 * 
	 * Inventory inv; int size = 0;
	 * 
	 * if (clickedBlock.getState() instanceof Chest) { size = 1; inv = ((Chest)
	 * clickedBlock.getState()).getInventory();
	 * 
	 * } else if(clickedBlock.getState() instanceof DoubleChest){ size = 2; inv
	 * = ((DoubleChest) clickedBlock.getState()).getInventory();
	 * 
	 * } else return;
	 * 
	 * inv.clear(); Random r = new Random();
	 * 
	 * for(ItemStack i: ChestRatioStorageOLD.getInstance().getItems()){ int l =
	 * r.nextInt(26 * size);
	 * 
	 * while(inv.getItem(l) != null) l = r.nextInt(26 * size); inv.setItem(l,
	 * i);
	 * 
	 * 
	 * } openedChest3.add(clickedBlock); GameManager.openedChest.put(gameid,
	 * openedChest3); } catch(Exception e5){}
	 */
